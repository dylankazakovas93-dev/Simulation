...
class _AccountMachine:
    ...
    def apply_trade(self, trade: Trade) -> bool:
        ...
        if day != self._current_day:
            # For END-OF-DAY trailing, the drawdown threshold ratchets only at
            # session close: commit the just-closed day's CLOSING balance into the
            # peak before opening the new day. Intraday highs never raise the floor.
            if self.rules.trailing_basis == "end_of_day" and self._current_day is not None:
                self.peak = max(self.peak, self.balance)
            self._reset_daily(day, self.balance)

        pnl = self.rules.contracts_per_trade * float(trade.pnl_dollars)
        self.balance += pnl
        # END-OF-TRADE trailing ratchets on every realized fill; END-OF-DAY waits
        # for the day-rollover commit above.
        if self.rules.trailing_basis == "end_of_trade" and self.balance > self.peak:
...
def funded_window_analysis(
    ...
    *,
    ...
    # The account is ALREADY funded here, so it must not re-pay the evaluation or
    # activation fee — those are one-off entry costs accounted for separately. Zero
    # them so funded net cash is pure payouts (Review 015 fix).
    rules = _replace(rules, evaluation_fee=0.0, activation_fee=0.0)
